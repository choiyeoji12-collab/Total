...
interface fifo_interface (
    ...
    logic       we;  //push
    logic       re;  //pop
    logic [7:0] wdata;  // push_data
    logic [7:0] rdata;  // pop_data
    logic       full;
    logic       empty;
...
class generator;
    ...
    transcation tr;
    mailbox #(transcation) gen2drv_mbox;
    event gen_next_ev;

    function new(mailbox#(transcation) gen2drv_mbox, event gen_next_ev);
...
class driver;
    ...
    function new(mailbox#(transcation) gen2drv_mbox,
                 virtual fifo_interface fifo_if);
        this.gen2drv_mbox = gen2drv_mbox;
...
class monitor;
    transcation tr;
    mailbox #(transcation) mon2scb_mbox;
    virtual fifo_interface fifo_if;

    function new(mailbox#(transcation) mon2scb_mbox,
                 virtual fifo_interface fifo_if);
...
class scoreboard;
    ...
    function new(mailbox#(transcation) mon2scb_mbox, event gen_next_ev);
        this.mon2scb_mbox = mon2scb_mbox;
        this.gen_next_ev  = gen_next_ev;
    endfunction  //new()
...
module tb_fifo_sv ();
    ...
    fifo_sv dut (
        ...
        .we   (fifo_if.we),     //push
        .re   (fifo_if.re),     //pop
        .wdata(fifo_if.wdata),  // push_data
        .rdata(fifo_if.rdata),  // pop_data
        .full (fifo_if.full),
        .empty(fifo_if.empty)
    );
